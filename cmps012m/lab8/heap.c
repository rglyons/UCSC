...
void insertHeap(heap *aHeap, int value) {
    ...
    int position = aHeap->size;
    aHeap->array[position] = value;
    while (aHeap->array[position] > aHeap->array[parent(position)]) {
	int temp = aHeap->array[position];
	aHeap->array[position] = aHeap->array[parent(position)];
	aHeap->array[parent(position)] = temp;
